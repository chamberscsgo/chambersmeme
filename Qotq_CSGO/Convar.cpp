...
public:
	virtual bool RegisterConCommandBase ( ConCommandBase *pVar )
	{
		...
		g_pICvar->RegisterConCommand ( pVar );
		return true;
	}
};
...
{
	if ( !g_pICvar || s_bRegistered )
		return;

	assert ( s_nDLLIdentifier < 0 );
	s_bRegistered = true;
	s_nCVarFlag = nCVarFlag;
	s_nDLLIdentifier = g_pICvar->AllocateDLLIdentifier ( );
...
void ConVar_Unregister ( )
{
	...
	assert ( s_nDLLIdentifier >= 0 );
	g_pICvar->UnregisterConCommands ( s_nDLLIdentifier );
	s_nDLLIdentifier = -1;
	s_bRegistered = false;
}
...
{
	if ( g_pICvar ) {
		g_pICvar->UnregisterConCommand ( this );
	}
}
...
void ConVar::ChangeStringValue ( const char *tempVal , float flOldValue )
{
	...
	if ( g_pICvar )
		g_pICvar->CallGlobalChangeCallbacks ( this , pszOldValue , flOldValue );
}
...
{
	m_pOriginalCVar = g_pICvar->FindVar ( szCVar );
	Spoof ( );
}
...
SpoofedConvar::SpoofedConvar ( ConVar* pCVar )
{
	...
	if ( !IsSpoofed ( ) && m_pOriginalCVar )
	{
		...
		//Register it
		g_pICvar->RegisterConCommand ( m_pDummyCVar );
...
void SpoofedConvar::Restore ( )
{
	if ( IsSpoofed ( ) )
	{
		...
		g_pICvar->UnregisterConCommand ( m_pDummyCVar );
