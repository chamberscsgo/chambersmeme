...
{
	static DWORD ClipTraceToPlayersAdd = ( DWORD ) g_pUtils->FindPatternIDA ( ( "client.dll" ) , "53 8B DC 83 EC 08 83 E4 F0 83 C4 04 55 8B 6B 04 89 6C 24 04 8B EC 81 EC ? ? ? ? 8B 43 10" );
...
bool Autowall::IsBreakableEntity ( IClientEntity* entity )
{
	...
	static isBreakbaleEntityFn IsBreakableEntityFn = ( isBreakbaleEntityFn ) g_pUtils->FindPatternIDA ( ( "client.dll" ) , "55 8B EC 51 56 8B F1 85 F6 74 68" );
	...
	if ( IsBreakableEntityFn )
	{
		...
		if ( ( C_BaseEntity* ) entity != g_pEntityList->GetClientEntity ( 0 ) )
		{
...
void Autowall::ScaleDamage ( CGameTrace &enterTrace , CCSWeaponInfo *weaponData , float& currentDamage )
{
	...
	bool hasHeavyArmor = ( ( C_BaseEntity* ) enterTrace.hit_entity )->HeavyArmor ( );
...
{
	return ent != NULL && ( C_BaseEntity* ) ent == g_pEntityList->GetClientEntity ( 0 );
}
...
bool Autowall::TraceToExit ( CGameTrace& enterTrace , CGameTrace& exitTrace , Vector startPosition , Vector direction )
{
	...
	*/
	...
	while ( currentDistance <= maxDistance )
	{
		...
		if ( !firstContents )
			firstContents = g_pEngineTrace->GetPointContents ( start , 0x4600400B , nullptr ); /*0x4600400B*/
		int pointContents = g_pEngineTrace->GetPointContents ( start , 0x4600400B , nullptr );
...
bool Autowall::HandleBulletPenetration ( CCSWeaponInfo* weaponData , CGameTrace& enterTrace , Vector& eyePosition , Vector direction , int& possibleHitsRemaining , float& currentDamage , float penetrationPower , bool sv_penetration_type , float ff_damage_reduction_bullets , float ff_damage_bullet_penetration )
{
	...
	if ( sv_penetration_type )
	{
		...
		else if ( enterMaterial == CHAR_TEX_FLESH && ( local->GetTeam ( ) == pEnemy->GetTeam ( ) && ff_damage_reduction_bullets == 0.f ) ) //TODO: Team check config
		{
...
bool Autowall::FireBullet ( C_BaseWeapon* pWeapon , Vector& direction , float& currentDamage )
{
	if ( !pWeapon )
		return false;
	auto local = g_pEntityList->GetClientEntity ( g_pEngine->GetLocalPlayer ( ) );
	...
	static ConVar* penetrationSystem = g_pICvar->FindVar ( "sv_penetration_type" );
	static ConVar* damageReductionBullets = g_pICvar->FindVar ( "ff_damage_reduction_bullets" );
	static ConVar* damageBulletPenetration = g_pICvar->FindVar ( "ff_damage_bullet_penetration" );
	...
	GetBulletTypeParameters ( penetrationPower , penetrationDistance , weaponData->bullet_type , sv_penetration_type );
	...
	while ( possibleHitsRemaining > 0 && currentDamage >= 1.f )
	{
		...
		bool onTeam = ( ( ( C_BaseEntity* ) enterTrace.hit_entity )->GetTeam ( ) == ( int32_t ) TEAM_COUNTER_TERRORIST || ( ( C_BaseEntity* ) enterTrace.hit_entity )->GetTeam ( ) == ( int32_t ) TEAM_TERRORIST );
...
float Autowall::CanHit ( Vector &point )
{
	...
	QAngle angle = Math::CalcAngle2 ( data.src , rem );
